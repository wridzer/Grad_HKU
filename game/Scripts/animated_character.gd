...
func attack_animation(type: String, level: int = 1) -> bool:
	...
	# Apply animation direction and parameter to activate animation
	animation_tree.set("parameters/" + type.capitalize() + "/blend_position", level)
	animation_tree.set("parameters/conditions/" + type, true)
	
	# Can't find a way to get the length from the Animation Tree
	var animation_length: float = animation_player.get_animation("character_animations/" + type + "_" + str(level)).length
